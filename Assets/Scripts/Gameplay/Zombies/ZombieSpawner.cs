...
namespace PvZ.Gameplay.Zombies
{
    ...
    public sealed class ZombieSpawner : SceneSingleton<ZombieSpawner>
    {
        ...
        [Header("Simple Automatic Spawn")]
        [SerializeField] private bool spawnAutomatically;
        [SerializeField] private GameConfigObject.ZombieType defaultZombieType =
        ...
        private Coroutine _automaticSpawnRoutine;
        ...
        public ZombieEntity SpawnZombie(GameConfigObject.ZombieType zombieType)
        {
            EnsureCanSpawn();
        ...
        public ZombieEntity SpawnZombieInRow(
            GameConfigObject.ZombieType zombieType,
            int row)
        {
            EnsureCanSpawn();
            ...
            var prefab = MainGameManager.Instance.GetZombieByType(zombieType);
        ...
        protected override void OnSingletonStart()
        {
            ...
            {
                _automaticSpawnRoutine = StartCoroutine(SpawnAutomatically());
            }
        }

        protected override void OnSingletonDestroy()
        {
            if (_automaticSpawnRoutine != null)
            {
                StopCoroutine(_automaticSpawnRoutine);
                _automaticSpawnRoutine = null;
            }
        ...
        private IEnumerator SpawnAutomatically()
        {
            ...
            {
                if (AliveZombieCount < maxAliveZombies)
                {
                    SpawnZombie(defaultZombieType);
                }
        ...
        private void EnsureCanSpawn()
        {
            if (MainGameManager.Instance == null)
            {
                throw new MissingReferenceException(
                    $"{nameof(ZombieSpawner)} requires an active {nameof(MainGameManager)} in the scene.");
            }
